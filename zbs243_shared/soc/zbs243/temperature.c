...
int8_t adcSampleTemperature(void) __reentrant {
    uint16_t sum = 0;
    uint8_t i;
    ...
    */
    ...
    if (!flashRead(FLASH_INFOPAGE_ADDR + 0x0b, mTempRet, 4) || (mTempRet[0] == 0xff && mTempRet[1] == 0xff) || (mTempRet[2] == 0xff && mTempRet[3] == 0xff)) {
        ...
        uint16_t valB = mathPrvU16from2xU8(mTempRet[0], mTempRet[1]);  // temp sensor reading at 30 degrees C
        uint16_t valD = mathPrvU16from2xU8(mTempRet[2], mTempRet[3]);  // temp sensor reading at 45 degrees C
        uint16_t slope = valD - valB;
